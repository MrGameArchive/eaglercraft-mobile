...
// Create a hidden input field to focus
keyboardButton.addEventListener("touchstart", function(e) {
    e.preventDefault(); // You can leave this if you need to prevent other behaviors
}, false);

keyboardButton.addEventListener("touchend", function(e) {
    e.preventDefault(); // Prevent other behaviors

    // Focus on the hidden input to trigger the keyboard
    if (window.hiddenInputFocused) {
        hiddenInput.blur();
    } else {
        hiddenInput.focus(); // Focus the hidden input
        window.hiddenInputFocused = true;  // Mark as focused
    }
}, false);
